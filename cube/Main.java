...
public class Main implements Observer {
    ...
    private void initPuzzle() {
        int row = _random.nextInt(_side);
        int col = _random.nextInt(_side);
        boolean[][] colored = new boolean[_side][_side];
        int counter = 0;
        while (counter < 6) {
            int randR = _random.nextInt(_side);
            int randC = _random.nextInt(_side);
            if (!colored[randR][randC] && randR != row && randC != col) {
                colored[randR][randC] = true;
                counter += 1;
            }
        }
        _model.initialize(_side, row, col, colored);
        _done = false;
    }
